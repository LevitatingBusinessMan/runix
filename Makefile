iso: link
	...
	cp target/kernel.bin target/isofiles/boot/kernel.bin
...
link: boot kernel
	ld -n -o target/kernel.bin -T link.ld target/multiboot_header.o target/boot.o target/x86-runix/debug/librunix.a
